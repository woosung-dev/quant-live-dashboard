...
export function CloudBotList() {
    ...
    const openDetailPanel = (botId: string) => {
        ...
        <>
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                ...
                {bots.map(bot => (
                    <Card
                        key={bot.id}
                        className={`border-l-4 cursor-pointer hover:shadow-lg transition-shadow ${bot.status === 'ACTIVE' ? 'border-l-green-500' : 'border-l-yellow-500'}`}
                        onClick={() => openDetailPanel(bot.id)}
                    >
